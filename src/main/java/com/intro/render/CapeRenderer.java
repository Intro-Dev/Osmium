...
import com.intro.CapeRenderingMode;
...
public class CapeRenderer extends FeatureRenderer<AbstractClientPlayerEntity, PlayerEntityModel<AbstractClientPlayerEntity>> {
    ...
    public static HashMap<String, Identifier> CapeArray = new HashMap<String, Identifier>();

    boolean dubugout = false;
    ...
    public void render(MatrixStack stack, VertexConsumerProvider vertexConsumers, int light, AbstractClientPlayerEntity entity, float limbAngle, float limbDistance, float tickDelta, float animationProgress, float headYaw, float headPitch) {
        if(!dubugout) {
            System.out.println("trying to render cape");
        }
        if(!entity.isInvisible() && entity.canRenderCapeTexture() && entity.isPartVisible(PlayerModelPart.CAPE) && (OsmiumOptions.CustomCapeModes == CapeRenderingMode.ALL || OsmiumOptions.CustomCapeModes == CapeRenderingMode.OPTIFINE)){
            MinecraftClient.getInstance().getProfiler().push("OsmiumCapeRender");
            if(!dubugout) {
                System.out.println("rendering cape");
                dubugout = true;
            }
            stack.push();
